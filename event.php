...
//student enrollment table have userid and course code 
//course code in student enroll associate with course code in course details 
// if student select one row it will get the course code of the student enroll which then get the info of the course code from course table 
...
<body>
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT fld_activity_id, fld_activity_name FROM tbl_activity_details");
        // $stmt->bindParam(':user_id', $user_id);
        $stmt->execute();
        $activity_results = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $activity_options = '';
        foreach ($activity_results as $activity) {
            $absence_id = $activity['fld_activity_id'];
            $activity_name = $activity['fld_activity_name'];
            $activity_options .= "<option value=\"$activity_name\" data-absence-id=\"$absence_id\">$activity_name</option>";
        }
    ...
    <div class="buttons">
        ...
        ?>
                <div class="buttons">
                    ...
                    <button class="btn btn-primary custom-btn" onclick="window.location.href='event.php?user_id=<?php echo $id; ?>&course_code=<?php echo $readrow['fld_course_code']; ?>'">EVENTS</button>
        ...
        if (isset($readrow) && !empty($readrow)) {
            ...
            $absence_title = isset($readrow['fld_activity_name']) ? $readrow['fld_activity_name'] : '';
        }
        ?>
    </div>
    ...
    <div>
        <button class="next" onclick="redirectToSubmission()">Next ></button>
    ...
    <div class="form-group">
        ...
        <form method="post" action="event_crud.php" enctype="multipart/form-data" id="submissionForm">
            <div class="boxes">
                <div class="box">
                    ...
                    <label for="event" style="display: block; text-align: left;margin-top: 10px;">SELECT EVENT (ISTAR) :</label>
                    <select style="margin-top: 10px" name="title" id="title" class="form-control" value="<?php if (isset($_GET['edit'])) echo $editrow['fld_activity_name']; ?>" required>
                    ...
                    <div class="textarea" style="margin-top: 20px; overflow: hidden; font-size: 15;">
                        ...
                        <textarea id="description" name="Description" rows="5" cols="40" style="margin-left: 0;"></textarea>
                    </div>
                </div>

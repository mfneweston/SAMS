...
<body>
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT fld_absence_id, fld_absence_title 
            FROM tbl_absence_title WHERE fld_absence_id LIKE 'PA%'");
        $stmt->execute();
        $personal_results = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $personal_options = '';
        foreach ($personal_results as $personal) {
            $absence_id = $personal['fld_absence_id'];
            $personal_name = $personal['fld_absence_title'];
            $personal_options .= "<option value=\"$personal_name\" data-absence-id=\"$absence_id\">$personal_name</option>";
        }
    ...
    <div class="buttons">
        ...
        ?>
                <div class="buttons">
                    ...
                    <button class="btn btn-primary custom-btn" onclick="window.location.href='personal.php?user_id=<?php echo $id; ?>&course_code=<?php echo $readrow['fld_course_code']; ?>'">PERSONAL</button>
                </div>
        <?php
            }
        } else {
    ...
    <div class="form-group">
        ...
        <form method="post" action="submission.php" enctype="multipart/form-data" id="submissionForm">
            <div class="boxes">
                <div class="box">
                    ...
                    <select style="margin-top: 10px" name="title" class="form-control" id="title" value="<?php if (isset($_GET['edit'])) echo $editrow['fld_absence_title']; ?>" required>
                        <option value="">Select Personal Reason</option>
                        <?php echo $personal_options; ?>
                    </select>
                    ...
                    <div class="textarea" style="margin-top: 20px; overflow: hidden; font-size: 15;">
                        ...
                        <textarea id="description" name="Description" rows="5" cols="40" value="<?php if (isset($_GET['edit'])) echo $editrow['fld_absence_desc']; ?>" style="margin-left: 0;"></textarea>
                    </div>
                </div>
                <div class="box">

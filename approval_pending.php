...
$fld_status = isset($_GET['fld_status']) ? $_GET['fld_status'] : '';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['approve'])) {
        ...
        try {
            ...
            $update_stmt = $conn->prepare("UPDATE tbl_submission_details SET fld_status = :update_status WHERE fld_user_id = :user_id AND fld_submission_id = :submission_id");
            $update_stmt->bindParam(':update_status', $update_status);
            $update_stmt->bindParam(':user_id', $user_id);
            $update_stmt->bindParam(':submission_id', $submission_id);
            $update_stmt->execute();
            
            // Redirect to approval_accepted.php after updating status
            header("Location: approval_accepted.php?user_id=$user_id&user_name=" . urlencode($user_name) . "&submission_id=$submission_id&fld_status=$update_status");
            exit();
        ...
        try {
            ...
            // Redirect to approval_rejected.php after updating status
            header("Location: approval_rejected.php?user_id=$user_id&user_name=" . urlencode($user_name) . "&submission_id=$submission_id&fld_status=$update_status");
            exit();
        } catch (PDOException $e) {
            echo "Error: " . $e->getMessage();
        }
...
?>
  ...
  <head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" integrity="sha384-mQ93GR66B00ZXjt0YO5KlohRA5SY2XofGJopCU8hjzsL/6p6a5I1pXkPnL+2aa8" crossorigin="anonymous">
...
<button onclick="location.href='review_student_absence_activity.php'" class="next">BACK</button>

        <div class="logo">
          <img src="pictures/ukm-logo.png" alt="Your Image Description">
        </div>

        <div class="box-left" style="margin-left: 30px">
    ...
    <div class="buttons-approve" >
      <button name="approve" onclick="return confirmApprove(event)" type="submit" style="font-weight: bold; font-size: 15px; font-family: 'Poppins', sans-serif;" class="upload-button2">APPROVE</button>

    <div class="buttons-reject" >
     <button name="reject" onclick="return confirmReject(event)" type="submit" style="font-weight: bold; font-size: 15px; font-family: 'Poppins', sans-serif;" class="upload-button1">REJECT</button>
...
<script>
  function confirmApprove(event) {
    // Display a confirmation dialog when the user clicks the "APPROVE" button
    var confirmation = confirm("Are you sure you want to accept this student absence?");

    // If user clicks OK in the confirmation dialog, submit the form
    if (confirmation) {
  ...
  function confirmReject(event) {
    // Display a confirmation dialog when the user clicks the "REJECT" button
    var confirmation = confirm("Are you sure you want to reject this student absence?");

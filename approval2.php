...
?>
  ...
  <body>
    ...
    $submission_stmt = $conn->prepare("
        SELECT sd.*, cd.fld_course_name, cd.fld_time as course_time, cd.fld_course_duration, 
        ud_lecturer.fld_user_name as lecturer_name,
        ud_student.fld_user_name as student_name
        FROM tbl_submission_details sd
        JOIN tbl_student_enrollment se ON sd.fld_user_id = se.fld_user_id
        JOIN tbl_course_details cd ON sd.fld_course_code = cd.fld_course_code
        ...
        WHERE sd.fld_user_id = :user_id 
        AND sd.fld_course_code = :course_code
    ");
    $submission_stmt->bindParam(':user_id', $user_id);
    $submission_stmt->bindParam(':course_code', $course_code);
    $submission_stmt->execute();
    $result = $submission_stmt->fetchAll(PDO::FETCH_ASSOC);

    if ($result) {
        $tbl_submission_details = $result[0]; // Assuming you only want the first row
...
<button onclick="location.href='review_student_absence_activity.php'" class="next">BACK</button>
        ...
        <div class="box-left">
          ...
          <form method="POST" action="" enctype="multipart/form-data">
           ...
           <div class="info-section">
          <?php
          // Get the course id from tbl_submission_details
          $course_code= isset($tbl_submission_details['fld_course_code']) ? $tbl_submission_details['fld_course_code'] : '';

          // Fetch course name from tbl_course_details using the course id
          $course_name = '';
          if (!empty($course_code)) {
              $stmt = $conn->prepare("SELECT fld_course_name FROM tbl_course_details WHERE fld_course_code = :course_code");
              $stmt->bindParam(':course_code', $course_code);
              $stmt->execute();
              $course_result = $stmt->fetch(PDO::FETCH_ASSOC);

              if ($course_result) {
                  $course_name = $course_result['fld_course_name'];
              }
          }
          ?>
          <p><span style="font-weight: bold;">COURSE :</span> <?php echo $course_code; ?> <?php echo $course_name; ?></p>
          </div>
           <!-- get the date of absence from submission details table & time from tbl_course_details via course code in tbl_submission_details -->
          <div class="info-section">
              <?php
              $absence_date = isset($tbl_submission_details['fld_absence_date']) ? $tbl_submission_details['fld_absence_date'] : '';
              $course_time = isset($tbl_course_details['fld_time']) ? $tbl_course_details['fld_time'] : '';
              echo "<p><span style=\"font-weight: bold;\">DATE AND TIME: </span>{$absence_date} {$course_time}</p>";
              ?>
          </div>
        ...
        <div class="info-section">
        <p><span style="font-weight: bold;">SUPPORTED DOCUMENT : </span><span><?php echo isset($tbl_submission_details['fld_file_upload']) ? $tbl_submission_details['fld_file_upload'] : ''; ?></span>
    ...
    <div class="buttons-approve" >
      <button name="approve" type="submit" style="font-weight: bold; font-size: 15px; font-family: 'Poppins', sans-serif;" class="upload-button2">APPROVE</button>

    <div class="buttons-reject" >
      <button name="reject" type="submit" style="font-weight: bold; font-size: 15px; font-family: 'Poppins', sans-serif;" class="upload-button1">REJECT</button>
